...
name_datasets<-c()
for(i in 15:26){
  name_datasets[i-14]=paste("p1fc",i,sep="")
}

# Read the name of the column
nc<-scan("all_id.txt",what=character())

for(name_data in name_datasets){
  results<-data.frame()
  for (v in 1:30){
    accuracy=0
    ...
    n<-scan(paste(name_data,"_intersect_id.txt",sep=""),what=character())
    maxnumber=length(n)
    df<-df[(row.names(df) %in% n), ]
    df<-as.data.frame(t(df))
    tipo<-scan(paste("HDtrain",v,"_type.txt",sep=""),what=character())
    df["type"]<-tipo
    df$type=factor(df$type)
    
    # Read data of the test set
    nltest<-scan(paste("HDtest",v,"_samples.txt",sep=""),what=character())
    dftest<-read.table(paste("HDtest",v,".txt",sep=""),header=FALSE,sep="\t",dec=".",row.names=nltest,col.names=nc)
    dftest<-data.frame(t(dftest))
    dftest<-dftest[(row.names(dftest) %in% n), ]
    dftest<-data.frame(t(dftest))
    tipotest<-scan(paste("HDtest",v,"_type.txt",sep=""),what=character())
    ...
    for (j in c(125,250,375,500,625,750,875,1000)){
      ...
      write.csv2(tab,file=paste(name_data,"_",v,"_",j,"_randomFoest_results_tuning_without.txt",sep=""))
    }
    ...
    tabx["set"]<-c(paste("HDtrain",v,sep=""))
    tabx<-tabx[c("set","mtry","ntree","Accuracy","Balanced Accuracy","Specificity","Precision","Recall","F1","MCC","ROC","AUCPR","Kappa","Sensitivity","Pos Pred Value","Neg Pred Value","Prevalence","Detection Rate","Detection Prevalence","AccuracyLower","AccuracyUpper","AccuracyNull","AccuracyPValue","McnemarPValue")]
    results<-rbind(results,tabx)
    
    # Write the results
    write.csv2(varImp(rfn,scale=FALSE)$importance, file=paste(name_data,"_",v,"_varImp.txt",sep=""))
    capture.output(confmatrix,file=paste(name_data,"_",v,"_confmatrix.txt",sep=""))
    write.csv2(rfpred2,file=paste(name_data,"_",v,"_predprobability.txt",sep=""))
  }
  write.csv2(results,file=paste(name_data,"_results.txt",sep=""))
}
